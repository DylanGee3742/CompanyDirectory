<?php
    ...
    $alreadyExists = $conn->prepare('SELECT * FROM location WHERE name = ?');
	...
	if (false === $alreadyExists) {
		...
		//If there are any results then location already exists 
	if (!empty($location)) {
	...
	$output['status']['returnedIn'] = (microtime(true) - $executionStartTime) / 1000 . " ms";
	$output['data'] = 'Location already exists';
